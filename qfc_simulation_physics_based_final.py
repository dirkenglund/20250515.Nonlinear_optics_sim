...
class PPLNWaveguide:
    def __init__(self, 
                 ...
                 poling_std=0.01e-6,     # Standard deviation of poling period (tuned)
                 temp_gradient=0.2,      # Temperature gradient across waveguide (tuned)
    ...
    def phase_matching_for_single_peak(self, lambda_n, lambda_p, lambda_e, center_wavelength):
        """
        Calculate phase matching for a single peak with physical broadening mechanisms
        
        Parameters:
        - lambda_n: Networking wavelength (nm)
        - lambda_p: Pump wavelength (nm)
        - lambda_e: Emitter wavelength (nm)
        - center_wavelength: Center wavelength for this peak (nm)
        """
        # Calculate poling period that would phase-match at the center wavelength
        # This is a simplified approach to model different QPM orders or waveguide modes
        # In reality, these would come from solving the phase-matching equations
        
        # First, find the poling period that would phase-match at the center wavelength
        # by iteratively adjusting the poling period
        test_periods = np.linspace(self.poling_period * 0.95, self.poling_period * 1.05, 100)
        min_mismatch = float('inf')
        best_period = self.poling_period
        
        for period in test_periods:
            mismatch = abs(self.phase_mismatch(center_wavelength * 1e-9, lambda_p * 1e-9, lambda_e * 1e-9, period, self.temp))
            if mismatch < min_mismatch:
                min_mismatch = mismatch
                best_period = period
        
        # Now use this period with small variations to model fabrication imperfections
        poling_periods = np.linspace(best_period - 3*self.poling_std, best_period + 3*self.poling_std, 15)
        poling_weights = np.exp(-(poling_periods - best_period)**2 / (2 * self.poling_std**2))
        poling_weights = poling_weights / np.sum(poling_weights)  # Normalize
        
        # Create a temperature distribution to model thermal gradients
        temperatures = np.linspace(self.temp - self.temp_gradient/2, self.temp + self.temp_gradient/2, 5)
        ...
        for period, p_weight in zip(poling_periods, poling_weights):
            for temp in temperatures:
                delta_k = self.phase_mismatch(lambda_n * 1e-9, lambda_p * 1e-9, lambda_e * 1e-9, period, temp)
    ...
    def multi_peak_phase_matching(self, lambda_n_range, lambda_p, lambda_e):
        ...
        # Each peak corresponds to a different QPM condition or waveguide mode
        peak_configs = [
            {"center": 1397.5, "weight": 1.0},  # Primary peak
            {"center": 1395.0, "weight": 0.6},  # Secondary peak
            {"center": 1400.0, "weight": 0.6},  # Secondary peak
            {"center": 1393.0, "weight": 0.3}   # Tertiary peak
        ]
        ...
        for config in peak_configs:
            ...
            for i, lambda_n in enumerate(lambda_n_range):
                # Use physically accurate broadening model for this peak
                response = self.phase_matching_for_single_peak(
                    lambda_n, lambda_p, lambda_e, center)
        ...
        # This scaling represents the coupling efficiency and other system factors
        scaling = 10.0  # Adjusted to match experimental peak heights
