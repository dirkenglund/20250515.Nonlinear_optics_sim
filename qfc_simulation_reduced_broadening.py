...
class PPLNWaveguide:
    def __init__(self, 
                 ...
                 poling_std=0.002e-6,    # Standard deviation of poling period (reduced)
                 effective_length_factor=0.5,  # Effective length factor due to domain disorder
                 mode_coupling=0.1,      # Mode coupling coefficient
                 domain_correlation=0.3, # Domain correlation factor (0-1)
                 temp_gradient=0.5,      # Temperature gradient across waveguide (°C)
                 apodization_factor=0.2, # Apodization factor for domain boundaries
                 duty_cycle_variation=0.05): # Duty cycle variation from ideal 50%
    ...
    def phase_matching_function(self, delta_k, effective_length):
        ...
        asymmetry = 1.0 + 0.1 * np.tanh(delta_k * self.duty_cycle_variation * 1e-5)
    ...
    def calculate_single_peak(self, lambda_n_range, lambda_p, lambda_e, center_wavelength, mode_index=0):
        ...
        z_positions = np.linspace(0, self.length, 5)
        ...
        if mode_index > 0:
            ...
            kernel_width = int(2 * mode_index) + 1  # Ensure odd width
            if kernel_width > 1:
    ...
    def multi_peak_phase_matching(self, lambda_n_range, lambda_p, lambda_e):
        ...
        kernel_width = 7  # Reduced from previous value
        kernel = np.exp(-np.linspace(-2, 2, kernel_width)**2)
        kernel = kernel / np.sum(kernel)  # Normalize
...
def simulate_phase_matching_spectrum():
    ...
    plt.savefig('phase_matching_spectrum_reduced_broadening.png', dpi=300, bbox_inches='tight')
    
    return 'phase_matching_spectrum_reduced_broadening.png'
...
def create_overlay_comparison():
    ...
    # Create wavelength range for net
